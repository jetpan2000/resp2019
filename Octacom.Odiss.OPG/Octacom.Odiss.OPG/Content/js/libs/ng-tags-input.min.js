...
THE DEFAULT CODE WAS CHANGED. CHECK THE CHANGES BEFORE UPDATING TO A NEW VERSION.
 - Allow multiple value selection separated by comma
        ...
        self.add = function(tag) {
            ...
            return canAddTag(tag)
                .then(function () {
                    ...
                    var mainTag = tag;
                    var moreTagsIn1 = mainTag[options.displayProperty].split(options.pasteSplitPattern);

                    if (moreTagsIn1.length > 1)
                    {
                        var allTags = [];

                        for (var i = 0; i < moreTagsIn1.length; i++)
                        {
                            var newTag = angular.copy(mainTag);
                            newTag[options.displayProperty] = moreTagsIn1[i];
                            
                            self.items.push(newTag);
                            events.trigger('tag-added', { $tag: newTag });
                        }
                    }
                    else
                    {
